...
class Bot(Client):
    ...
    async def file_processor_worker(self):
        """Processes files from the queue with smart setup checks."""
        ...
        while True:
            try:
                ...
                if not user or not user.get('db_channels') or not user.get('post_channels'):
                    logger.warning(f"User {user_id} has not completed channel setup. Sending guidance.")
                    try:
                        await self.send_message(
                            user_id,
                            "Hello! To get started, you need to set up your channels first.\n\n"
                            "Please use the `/start` command to access the settings menu and add your **Database Channel** and at least one **Post Channel**.",
                ...
                if not self.owner_db_channel_id:
                    logger.error("Owner DB not set. Worker is sleeping. Please set it via admin panel.")
    ...
    async def process_batch_task(self, user_id, batch_key):
        """Waits and posts a batch of files with smart error handling."""
        try:
            ...
            async with self.batch_locks[user_id][batch_key]:
                ...
                for channel_id in user.get('post_channels', []).copy():
                    try:
                        ...
                        logger.warning(f"Poster URL failed for {channel_id}. Sending as text.")
                        await self.send_message(channel_id, caption, reply_markup=footer_keyboard, disable_web_page_preview=True)
    ...
    async def start(self):
        ...
        if self.owner_db_channel_id:
            logger.info(f"Loaded Owner DB ID [{self.owner_db_channel_id}]")
        else:
            logger.warning("Owner DB ID not set. Please use admin panel.")
